...
import {
  formatCurrency, formatDateTime, getStatusStep, ORDER_STATUS_LABELS, ORDER_STATUS_STEPS,
  ...
  return (
    <div className="flex items-center overflow-x-auto pb-1">
      {steps.map((step, i) => {
        ...
        return (
          <div key={step} className="flex items-center flex-1 min-w-[84px]">
            <div className="flex flex-col items-center flex-shrink-0">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center border-2 transition-colors ${
                cancelled ? 'border-neutral-200 bg-white' :
                done ? 'bg-brand-500 border-brand-500' :
                active ? 'border-brand-500 bg-white' :
                'border-neutral-200 bg-white'
              }`}>
                {!cancelled && done ? <CheckCircle size={14} className="text-white" /> :
                 !cancelled && active ? <div className="w-2.5 h-2.5 rounded-full bg-brand-500 animate-pulse" /> :
                 <div className="w-2 h-2 rounded-full bg-neutral-200" />}
              </div>
              <span className={`text-xs mt-1.5 font-medium text-center leading-tight ${
                !cancelled && i <= currentIdx ? 'text-brand-600' : 'text-neutral-400'
              }`}>
                {ORDER_STATUS_LABELS[step]}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={`h-0.5 flex-1 mx-1 mb-4 ${!cancelled && done ? 'bg-brand-500' : 'bg-neutral-200'}`} />
            )}
          </div>
        );
      })}
