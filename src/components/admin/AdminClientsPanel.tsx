...
export default function AdminClientsPanel({ users, orders }: Props) {
  ...
  const clients = useMemo(() => {
    ...
    const rows: ClientRow[] = users
      ...
      .map((profile) => {
        const clientOrders = (byUser.get(profile.id) ?? []).sort(
          (a, b) =>
      ...
      .sort((a, b) => {
        ...
        return (
          ...
          (a.lastOrderAt ? Date.parse(a.lastOrderAt) : 0)
        );
      });
  ...
  const currentPage = Math.min(
    page,
    Math.max(0, Math.ceil(clients.length / 25) - 1),
  );
    ...
    <div className="space-y-4">
      ...
      <div className="admin-directory">
        {clients.length === 0 ? (
          ...
          clients.slice(currentPage * 25, currentPage * 25 + 25).map((row) => {
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-3 text-xs flex-shrink-0">
                    ...
                    <span className="tabular-nums">
                      ...
                      <span
                        className={`font-bold ${row.activeCount ? "text-amber-800" : "text-black"}`}
                      >
                        {row.activeCount}
                      </span>
                    </span>
                  ...
                  <div className="border-t border-atlas-border px-5 py-4 space-y-3 bg-neutral-50/40">
                    ...
                    {row.orders.length === 0 ? (
                      ...
                      <div className="divide-y divide-atlas-border rounded-lg border border-atlas-border bg-white overflow-hidden">
                        {row.orders.slice(0, historyLimit).map((order) => {
                          ...
                          return (
                            ...
                            >
                              <span
                                className={`w-1.5 self-stretch rounded-full ${styles.banner}`}
                              />
                              <div className="min-w-0 flex-1">
                                <Link
                        ...
                        {row.orders.length > historyLimit && (
                          <button
                            className="admin-rail-link"
                            onClick={() =>
                              setHistoryLimit((limit) => limit + 25)
                            }
                          >
                            Show older orders (
                            {row.orders.length - historyLimit} remaining)
                          </button>
                        )}
                      </div>
                    )}
      ...
      <div className="admin-table-footer">
        ...
        <div className="admin-pagination">
          ...
          <button
            disabled={(currentPage + 1) * 25 >= clients.length}
            onClick={() => setPage(currentPage + 1)}
          >
            Next
          </button>
